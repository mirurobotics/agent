...
mod tests {
    ...
    impl Stores {
        ...
        async fn find_deployed(&self) -> Result<Option<Deployment>, ScanErr> {
            super::find_deployed(&self.deployments).await
        }

        async fn get_dpl_upload_rules(
            &self,
            deployment: Deployment,
        ) -> Result<Vec<UploadRule>, ScanErr> {
            super::get_dpl_upload_rules(&self.releases, &self.upload_rules, deployment).await
        }
    }
    ...
    mod find_deployed {
        ...
        async fn find_deployed_returns_deployed() {
            ...
            let deployed = stores.find_deployed().await.unwrap();
        ...
        async fn find_deployed_none_when_only_queued() {
            ...
            assert!(stores.find_deployed().await.unwrap().is_none());
        }
        ...
        async fn find_deployed_errors_when_store_shut_down() {
            let stores = Stores::new().await;
            stores.deployments.shutdown().await.unwrap();

            let err = stores.find_deployed().await.unwrap_err();
            assert!(matches!(err, ScanErr::CacheErr(_)));
        }
    }

    mod get_dpl_upload_rules {
        use super::*;
        ...
        async fn resolves_active_set() {
            ...
            let rules = stores.get_dpl_upload_rules(deployed).await.unwrap();
            assert_eq!(
                ids(&rules),
        ...
        async fn missing_rule_body_errors() {
            ...
            let deployed = stores.find_deployed().await.unwrap().expect("deployed");
            let err = stores.get_dpl_upload_rules(deployed).await.unwrap_err();
