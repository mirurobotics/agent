...
pub async fn events(
    ...
    let replayed: Vec<Event> = event_hub
        .replay_after(cursor)
        .await
    ...
    let replayed: Vec<Event> = match &type_filter {
    ...
    let replay_stream = tokio_stream::iter(replayed.into_iter().map(Ok::<Event, Infallible>));

    let live_stream = BroadcastStream::new(broadcast_rx)
        .filter_map(|result: Result<Event, _>| result.ok())
        .filter(move |envelope: &Event| envelope.id > last_replayed_id);

    let type_filter_live = type_filter;
    let combined = replay_stream
        .chain(live_stream.map(Ok::<Event, Infallible>))
        .filter_map(move |result: Result<Event, Infallible>| {
