...
/// digest) became deletable at `eligible_at`; delete it once `ttl_secs` have
...
impl PendingDelete {
    ...
    /// TTL. A TTL too large to represent saturates to the far future — the
    /// record is simply never due (the backend bounds `ttl_secs` on the wire,
    /// so this is defense in depth, not a reachable policy).
