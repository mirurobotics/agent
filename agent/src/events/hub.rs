...
enum Command {
    ...
    Publish {
        event: NewEvent,
        respond_to: oneshot::Sender<Result<Envelope, EventsErr>>,
    },
    ReplayAfter {
        cursor: u64,
        respond_to: oneshot::Sender<Result<Vec<Envelope>, EventsErr>>,
    },
}
...
impl EventHub {
    ...
    pub async fn publish(&self, event: NewEvent) -> Result<Envelope, EventsErr> {
    ...
    pub async fn replay_after(&self, cursor: u64) -> Result<Vec<Envelope>, EventsErr> {
    ...
    pub async fn try_publish(&self, event: NewEvent) {
