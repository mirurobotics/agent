...
//! `J: QueueJob`. Each worker's own test file (`upload/queue.rs`,
//! `retention/queue.rs`) invokes [`queue_suite!`] with its job factory, which
//! emits one `#[tokio::test]` per case named exactly after the case function —
//! so a failing test name greps straight to its body here. The suite therefore
//! runs against `upload::Job` (always due on arrival) and `retention::Job`
//! (due only once its TTL elapses), proving the `due_at` hook in both states.
//!
...
/// The case bodies. Each is named exactly what its generated `#[tokio::test]`
/// is called, and takes one of two uniform signatures:
///
/// - disk-backed: `(tmp: &str, make: fn(&str) -> J)` — `tmp` names the temp dir
/// - in-memory: `(make: fn(&str) -> J)`
///
/// [`super::queue_suite`] holds the two lists and emits the wrappers.
pub mod cases {
    use super::*;

    // ------------------------------- disk-backed --------------------------------- //

    /// The factory is unused: the point of the case is that a snapshot with no
    /// entries loads as an empty queue rather than as anything else. It is taken
    /// anyway so the suite has one uniform signature, and so `J` is inferred.
    pub async fn empty_snapshot_loads_empty_queue<J: QueueJob>(tmp: &str, _make: fn(&str) -> J) {
    ...
    /// capacity gates only new enqueues, so an over-capacity backlog drains
    /// before the queue accepts more. Truncating the load would silently drop
    /// queued jobs, since the next mutation rewrites the whole snapshot.
    pub async fn from_snapshot_loads_an_over_capacity_backlog<J: QueueJob>(
        tmp: &str,
        make: fn(&str) -> J,
    ) {
        let (_dir, path) = temp_path(tmp);
    ...
    pub async fn enqueue_persists<J: QueueJob>(tmp: &str, make: fn(&str) -> J) {
        let (_dir, path) = temp_path(tmp);
    ...
    /// A persist landing between an entry's selection and its resolution must
    /// not write the selected entry out of the snapshot. Stronger than
    /// [`leaves_the_entry_on_disk_until_removed`], which only pins that
    ...
    /// work to completion inside its match arm — so the interleaving is
    /// performed directly on the queue. The test exists so that making a run
    /// loop responsive to shutdown via `select!` cannot silently reintroduce
    /// the loss.
    ...
    pub async fn requeue_order_survives_a_reload<J: QueueJob>(tmp: &str, make: fn(&str) -> J) {
        let (_dir, path) = temp_path(tmp);
        let id;
    ...
    pub async fn reset_invalid_deadlines_persists<J: QueueJob>(tmp: &str, make: fn(&str) -> J) {
        let (_dir, path) = temp_path(tmp);
    ...
    /// An entry written before a field existed still loads, rather than failing
    /// to deserialize — which `new_with_default` would turn into a silent wipe.
    ...
    pub async fn enqueue_appends_in_fifo_order<J: QueueJob>(make: fn(&str) -> J) {
    ...
    /// Same-job is not a capacity bypass: a duplicate would grow the queue just
    /// as a new job would, so it is rejected too.
    ...
    pub async fn next_ready_skips_waiting_entries<J: QueueJob>(make: fn(&str) -> J) {
        let mut queue = Queue::<J>::new(DEFAULT_CAPACITY);
        let deadline = now() + TimeDelta::hours(1);
        queue
            .requeue(QueueEntry {
    ...
    /// The factory is unused: the case is about a queue that never held a job.
    /// It is taken anyway so the suite has one uniform signature, and so `J` is
    /// inferred.
    pub async fn next_ready_is_none_when_empty<J: QueueJob>(_make: fn(&str) -> J) {
    ...
    pub async fn removing_one_duplicate_leaves_the_other<J: QueueJob>(make: fn(&str) -> J) {
        let mut queue = Queue::<J>::new(DEFAULT_CAPACITY);
        let job = make("a.log");
        enqueue(&mut queue, job.clone()).await;
        enqueue(&mut queue, job).await;
    ...
    pub async fn requeue_rotates_to_the_tail<J: QueueJob>(make: fn(&str) -> J) {
    ...
    /// A requeue at capacity is admitted, and an entry the queue does not yet
    /// hold is simply appended.
    pub async fn requeue_at_capacity_admits_a_new_entry<J: QueueJob>(make: fn(&str) -> J) {
        let mut queue = Queue::<J>::new(1);
        enqueue(&mut queue, make("a.log")).await;
    ...
    pub async fn count_ready_counts_only_ready_entries<J: QueueJob>(make: fn(&str) -> J) {
        let mut queue = Queue::<J>::new(DEFAULT_CAPACITY);
    ...
    pub async fn reset_invalid_deadlines_pulls_back_only_beyond_the_horizon<J: QueueJob>(
        make: fn(&str) -> J,
    ) {
        let mut queue = Queue::<J>::new(DEFAULT_CAPACITY);
        let horizon = now() + TimeDelta::hours(24);
    ...
    pub async fn reset_invalid_deadlines_is_a_noop_when_all_inside<J: QueueJob>(
        make: fn(&str) -> J,
    ) {
        let mut queue = Queue::<J>::new(DEFAULT_CAPACITY);
        let horizon = now() + TimeDelta::hours(24);
    ...
    pub async fn earliest_next_attempt_returns_the_minimum<J: QueueJob>(make: fn(&str) -> J) {
        let mut queue = Queue::<J>::new(DEFAULT_CAPACITY);
...
/// Emits one `#[tokio::test]` per named case. `disk` cases are called with
/// `($tmp, $make)`, `mem` cases with `($make)`. Each generated test is named
/// exactly after the case it calls.
macro_rules! queue_suite_emit {
    (
        ...
        $(
            #[tokio::test]
            async fn $disk() {
                $crate::data_uploads::queue::cases::$disk($tmp, $make).await;
            }
        )*
        $(
            #[tokio::test]
            async fn $mem() {
                $crate::data_uploads::queue::cases::$mem($make).await;
            }
        )*
    };
}
...
macro_rules! queue_suite {
    ($make:expr, $tmp:literal) => {
        $crate::data_uploads::queue::queue_suite_emit! {
            $make, $tmp,
            disk: [
                empty_snapshot_loads_empty_queue,
                from_snapshot_loads_an_over_capacity_backlog,
                enqueue_persists,
                rejection_does_not_persist,
                persist_failure_is_swallowed,
                leaves_the_entry_on_disk_until_removed,
                persist_during_an_in_flight_entry_keeps_it_on_disk,
                requeue_order_survives_a_reload,
                attempts_and_deadline_survive_a_reload,
                reset_invalid_deadlines_persists,
                missing_optional_fields_default,
            ],
            mem: [
                enqueue_appends_in_fifo_order,
                ...
                next_ready_skips_waiting_entries,
                next_ready_is_none_when_empty,
                unknown_id_is_ignored,
                removing_one_duplicate_leaves_the_other,
                requeue_rotates_to_the_tail,
                requeue_at_capacity_admits_a_new_entry,
                count_ready_counts_only_ready_entries,
                reset_invalid_deadlines_pulls_back_only_beyond_the_horizon,
                reset_invalid_deadlines_is_a_noop_when_all_inside,
                earliest_next_attempt_returns_the_minimum,
            ],
        }
    };
}
