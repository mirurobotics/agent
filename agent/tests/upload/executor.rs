...
#[tokio::test]
async fn after_upload_enqueues_pending_delete_and_keeps_source() {
    let client = Arc::new(MockClient::default());
    let (src, deleter, executor) = delete_policy_setup(client).await;
    let mut job = make_job("a.log");
    job.file = src.file().clone();
    job.delete_policy = DeletePolicy::AfterUpload;
    job.delete_delay_secs = 300;
...
#[tokio::test]
async fn never_leaves_source_in_place_and_enqueues_nothing() {
    let client = Arc::new(MockClient::default());
    let (src, deleter, executor) = delete_policy_setup(client).await;
    let mut job = make_job("a.log");
    job.file = src.file().clone();
    ...
    // Never leaves the source file untouched and records no pending deletion.
    assert!(path.exists());
...
async fn enqueue_failure_after_confirm_still_succeeds() {
    let client = Arc::new(MockClient::default());
    let (src, deleter, executor) = delete_policy_setup(client.clone()).await;
...
#[tokio::test]
async fn after_upload_without_deleter_still_succeeds() {
    let client = Arc::new(MockClient::default());
    client.set_create_upload(|| Ok(pending_response()));
    client.set_confirm_upload(|| Ok(*uploaded_response().upload));
    let src = files::temp("upload-delete-policy-test").unwrap();
