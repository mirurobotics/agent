...
// LogLevel serde (deserialize, serialize, roundtrip, case-insensitivity,
// aliases, non-string fallback) and `Display` are covered by the inline
// `log_level_serde_tests` module generated by `impl_status_enum!`. The tests
// below cover behavior the macro does not generate: derived ordering, default,
// variant order, and the logging layer/reload machinery.
