...
use miru_agent::events::model::NewEvent;
...
impl Fixture {
    async fn new(name: &str, with_event_hub: bool) -> Self {
        ...
        let event_hub = if with_event_hub {
    ...
    fn event_hub(&self) -> &EventHub {
        self.state.event_hub.as_ref().unwrap()
    }
}

fn make_event(event_type: &str) -> NewEvent {
    NewEvent {
...
mod no_event_hub {
    ...
    #[tokio::test]
    async fn returns_503_when_event_hub_not_initialized() {
        let f = Fixture::new("sse_no_hub", false).await;
...
mod cursor {
    ...
    async fn malformed_after_param_returns_400() {
        let f = Fixture::new("sse_bad_cursor", true).await;
    ...
    async fn malformed_last_event_id_returns_400() {
        let f = Fixture::new("sse_bad_lei", true).await;
    ...
    async fn after_param_takes_precedence_over_last_event_id() {
        let f = Fixture::new("sse_after_precedence", true).await;
...
mod stream {
    ...
    async fn returns_200_with_sse_content() {
        let f = Fixture::new("sse_stream_200", true).await;
    ...
    async fn replays_from_last_event_id_header() {
        let f = Fixture::new("sse_last_event_id", true).await;
    ...
    async fn empty_stream_returns_200() {
        let f = Fixture::new("sse_empty", true).await;
...
mod type_filter {
    ...
    async fn filters_events_by_type() {
        let f = Fixture::new("sse_type_filter", true).await;
    ...
    async fn multiple_type_filter() {
        let f = Fixture::new("sse_multi_type", true).await;
