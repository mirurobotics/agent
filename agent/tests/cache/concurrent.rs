...
pub mod write_if_absent {
    ...
    pub async fn inserts_when_absent_impl<F, Fut, SingleThreadCacheT>(spawn_cache: F)
    where
        F: Fn() -> Fut + Clone,
        Fut: Future<
            Output = (
    ...
    pub async fn noop_when_present_impl<F, Fut, SingleThreadCacheT>(spawn_cache: F)
    where
        F: Fn() -> Fut + Clone,
        Fut: Future<
            Output = (
