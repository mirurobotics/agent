...
pub mod get_deployment {
    ...
    async fn not_found_returns_error() {
        let (_dir, dpl_stor) = setup("get_dpl_not_found").await;
...
pub mod get_deployment_fallback {
    ...
    async fn cache_miss_backend_hit_caches_value() {
        ...
        let result = dpl_svc::get(&dpl_stor, Some(&stub), "dpl_1".to_string())
            .await
            .unwrap();
        assert_eq!(result.id, "dpl_1");
    ...
    async fn cache_miss_backend_404_returns_not_found() {
        ...
        let result = dpl_svc::get(&dpl_stor, Some(&stub), "dpl_1".to_string()).await;
        assert!(matches!(
            result,
    ...
    async fn cache_miss_no_backend_returns_not_found() {
        let (_dir, dpl_stor) = setup("fb_dpl_no_backend").await;
