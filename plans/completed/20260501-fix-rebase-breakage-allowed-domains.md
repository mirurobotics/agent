...
- [~] (2026-05-01 implement) Milestone 4: `cargo check --package miru-agent --features test --all-targets` is clean. `bash scripts/preflight.sh` ran but reported `Preflight FAILED (lint=0 tests=1 tools_lint=0 tools_tests=0)` — lint and tools both pass, only the agent test phase failed. Per the orchestrator's instructions for this stage, the implement loop did NOT iterate on preflight; the next stage (preflight skill) owns the refine→preflight loop. Note: preflight applied auto-format/lint fixes to `main.rs` (drop unnecessary borrow on `&settings.backend.base_url.as_str()`), `provision.rs` (rustfmt line wrap), and `shared.rs` (use-stmt ordering); those edits are committed below alongside this milestone update.
...
- 2026-05-01 (Milestone 4 preflight): preflight reports clean lint and tools but a single failure in the agent `tests` phase. Compile is clean (`cargo check --features test --all-targets` succeeds), so the failure is a real runtime test failure, not a build problem. This implement-skill run does not investigate or repair it — per the orchestrator's contract, the next stage (preflight skill) owns the refine→preflight repair loop. Also: preflight applied a few auto-fixes (drop unneeded `&` on `&settings.backend.base_url.as_str()` in main.rs, rustfmt line-wrap in provision.rs's `mqtt_broker_host_override` test, and `use`-stmt alphabetical ordering in shared.rs); those are committed in the milestone-4 plan-update commit so the working tree is clean before handoff.
