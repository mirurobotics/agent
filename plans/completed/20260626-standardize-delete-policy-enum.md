...
### 2026-06-26 — Reverted to tests-only standardization (Milestones 1 & 2 backed out)

Per a follow-up decision, the macro extension (Milestone 1) and macro routing (Milestone 2) were **reverted**: carrying an ~85-line backend-only macro form for a single current caller wasn't worth the churn. `agent/src/models/status.rs` is restored to its `origin/main` state (no `@core` split, no backend-only form), and `DeletePolicy` keeps its hand-rolled `Display`/`Deserialize`/`From<&UploadDeletePolicy>` impls (with `Hash` added to the derive for the harness).

**Test standardization is retained** (the valued part): `DeletePolicy` uses the shared `status_serde_tests!` harness for its serde suite via a hand-written `StatusFixture` impl (inlined `variants()`/`wire_str()`, since the hand-rolled enum has no generated accessors). The backend→domain conversion and `Display` aren't covered by the harness, so explicit `from_backend_known`, `from_backend_unknown_defaults_to_never`, and `display` tests are kept alongside the harness call. `scripts/preflight.sh` → `Preflight clean`, `models` coverage = 100%. If a second backend-only enum (e.g. `UploadStatus`) later appears, revisit extending the macro then.
