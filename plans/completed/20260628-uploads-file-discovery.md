...
- [x] **M2.1** (2026-06-28) Added the `glob` dependency: `glob = "0.3.2"` in `[workspace.dependencies]` and `glob = { workspace = true }` in `agent/Cargo.toml`. `scripts/update-deps.sh` locked `glob v0.3.3` (semver-compatible). `cargo machete` confirms it is USED.
- [x] **M2.2** (2026-06-28) Added `agent/src/workers/uploads.rs` (`Options` + `Default`, `run(...)` with injected `sleep_fn` + injected `now_fn` clock + `shutdown_signal`, `run_impl` loop). Registered `pub mod uploads;` in `agent/src/workers/mod.rs`.
- [x] **M2.3** (2026-06-28) Per-rule poll cadence implemented (`next_scan_at: HashMap<UploadRuleID, DateTime<Utc>>`), reading rules each loop via `upload_rules.values().await` (cache error → logged + treated as empty). `next_scan_at` pruned for vanished rule ids.
- [x] **M2.4** (2026-06-28) Glob matching (`glob::glob(&rule.source.glob)`) + readiness state machine (per-file `size`/`mtime`/`stable_since`; reset on change; ready when unchanged and `now - stable_since >= stability_window_secs`). Blocking glob+stat runs under `tokio::task::spawn_blocking`. Finalization-marker HOOK is a comment only.
- [x] **M2.5** (2026-06-28) Placeholder log sink: newly-ready files emit `info!` `{ file_path, file_modified_at }`; an in-memory `already_reported: HashSet<PathBuf>` suppresses re-logging. Dedupe was moved INTO `decide_ready` (see Decision Log / Surprises) so the seam returns only newly-ready files; the sink stays wired (no dead code).
- [x] **M2.6** (2026-06-28) Wired into `agent/src/app/run.rs`: `enable_uploads_worker` (default `true`) + `uploads: uploads::Options` in `AppOptions`/`Default`; `init_uploads_worker`; `uploads_worker_handle` in `ShutdownManager` (new + register + shutdown step inserted after poller, before mqtt).
- [x] **M2.7** (2026-06-28) Tests — `agent/tests/workers/uploads.rs` (registered in `agent/tests/workers/mod.rs`): glob match/miss/recursive/invalid, stability state machine, per-rule cadence, once-only dedupe, empty/no-rules + cache-error idle. Added `register_handle_rejects_uploads_duplicates` in `app/run.rs`. New `mocks::clock::Clock` test helper. 8 uploads tests + full suite (1354 integration + 144 unit) green.
- [ ] **V** (PARTIAL, 2026-06-28) Per the M2 execution scope, full `scripts/preflight.sh` was NOT run (deferred to a later step). Verified clean: `cargo build -p miru-agent --features test`, `cargo clippy -p miru-agent --features test --all-targets -- -D warnings`, `cargo fmt -p miru-agent -- --check`, `cargo machete`, and `cargo test -p miru-agent --features test` (all pass). `scripts/update-deps.sh` ran cleanly. Covgate not measured here.
...
- **(IMPLEMENTATION, 2026-06-28) The `decide_ready` testable seam absorbed the dedupe** so it is unit-testable without log capture. The plan sketched `decide_ready(rule, observations, now) -> Vec<ready>` with the `already_reported` insert living in `run_impl`. But integration tests (`agent/tests/`) can only see `pub` items and have no non-log observable for the dedupe. Resolution: `decide_ready` is now `pub fn decide_ready(rule, observations, already_reported, now) -> Vec<ReadyFile>` and returns ONLY newly-ready files (inserting each into `already_reported` exactly once). `FileObservation` is `pub` with `pub` fields; `ReadyFile { pub path, pub modified_at }`. `run_impl` owns `already_reported` and the `info!` sink still fires per returned file (no dead code). This made T2/T4 deterministic pure-fn tests.
- **(IMPLEMENTATION, 2026-06-28) `glob` resolved to `0.3.3`** (semver-compatible with the `0.3.2` requirement) via `scripts/update-deps.sh`. `cargo machete` confirms `glob` is USED. No RUSTSEC advisory.
- **(IMPLEMENTATION, 2026-06-28) `spawn_blocking` integration**: `run_impl` `std::mem::take`s `observations` + `already_reported`, moves them (with a cloned rule) into `tokio::task::spawn_blocking`, and writes both back from the returned tuple `(observations, already_reported, ready)`. A `JoinError` is logged and the maps reset to empty (resets stability windows, never corrupts — `decide_ready` is panic-resistant via `continue` on every fallible fs call). `decide_ready` itself stays a plain sync fn.
- **(IMPLEMENTATION, 2026-06-28) `write_if_absent` signature** (verified in tests): `write_if_absent<F>(&self, key, value, is_dirty: F)` where `F: Fn(Option<&CacheEntry<K,V>>, &V) -> bool + Send + Sync + 'static`; tests pass `|_, _| false`. `UploadRules::spawn(64, layout.upload_rules(), 1000).await` (buffer_size, File, capacity).
- **(IMPLEMENTATION, 2026-06-28) Forcing a cache error is cleanly feasible**: `ConcurrentCache::shutdown().await` stops the actor, so subsequent `values()` returns `CacheErr::SendActorMessageErr`. The cache-error idle test holds an `Arc<UploadRules>` clone to call `shutdown()` while the worker runs — T5's cache-error subcase was implemented, not skipped.
- **(IMPLEMENTATION, 2026-06-28) Test clock helper** lives at `agent/tests/mocks/clock.rs` (`Clock` wraps `Arc<AtomicI64>` epoch seconds; `now_fn()` returns `impl Fn() -> DateTime<Utc>`; `advance(secs)`; derives `Clone` to share the atomic with the worker's `now_fn`). Registered `pub mod clock;` in `agent/tests/mocks/mod.rs`.
...
- **Decision: the placeholder sink is a real, wired-in `info!` + `already_reported` set mutation — not a stub — so the readiness computation is genuinely consumed (no dead code).** Rationale: the deadcode gate (`cargo-diet`) + clippy `dead_code` would flag a readiness result that drives nothing. Logging the result AND mutating the in-memory `already_reported: HashSet<PathBuf>` (which in turn gates future logging) makes "ready" an observable side effect. M3 swaps the `info!`/set for the digest+`POST /uploads` pipeline. The finalization-marker HOOK is a COMMENT in the readiness path (not a stub function), so it adds no dead code.
  Date/Author: 2026-06-28 / plan author.
- **Decision (IMPLEMENTATION, 2026-06-28): move the `already_reported` dedupe INTO `decide_ready`; make the seam + `FileObservation` `pub`.** Rationale: integration tests in `agent/tests/` can only access `pub` items and have no non-log observable for the dedupe, so testing dedupe through the worker loop would have required a log-capture dev-dependency (rejected by the plan). Making `decide_ready` return only newly-ready files (and own the `already_reported` insert) turns dedupe into a pure, deterministic, directly-unit-testable transition while keeping the `info!` sink wired in `run_impl` (no dead code; `already_reported` is still owned and threaded by `run_impl`). This is a refinement of, not a departure from, the plan's "return newly-ready files; run_impl logs" seam (the plan's T4 wording already said "logged/returned as newly-ready exactly ONCE").
  Date/Author: 2026-06-28 / implementer.

## Outcomes & Retrospective

(2026-06-28, implementation complete for M2 source + tests; full preflight deferred per execution scope.)

- **Final `Options` fields**: `Options { min_poll_interval_secs: i64 }`, `Default = 1` (sleep floor + fallback for non-positive `poll_interval_secs`). No other knobs needed for M2.
- **`glob` under `spawn_blocking`**: YES. `decide_ready` (sync, blocking glob walk + `std::fs::metadata`) runs inside `tokio::task::spawn_blocking`; `observations`/`already_reported` move in and back out via the returned tuple.
- **`now_fn` test-clock helper location**: `agent/tests/mocks/clock.rs` (`Clock`, shared `Arc<AtomicI64>`, `Clone`), registered in `agent/tests/mocks/mod.rs`. Production passes `chrono::Utc::now`.
- **Final `decide_ready` signature**: `pub fn decide_ready(rule: &UploadRule, observations: &mut HashMap<PathBuf, FileObservation>, already_reported: &mut HashSet<PathBuf>, now: DateTime<Utc>) -> Vec<ReadyFile>`. `pub struct FileObservation { pub size: u64, pub mtime: SystemTime, pub stable_since: DateTime<Utc> }`; `pub struct ReadyFile { pub path: PathBuf, pub modified_at: DateTime<Utc> }`.
- **Measured `workers` covgate %**: NOT measured — `scripts/covgate.sh`/`scripts/preflight.sh` were intentionally not run in this step (deferred). The 8 uploads tests cover the pure readiness/cadence/dedupe logic thoroughly (the highly-testable bulk of `uploads.rs`); a later preflight step must confirm `agent/src/workers` stays `>= 83.21`.
- **Validation status**: `cargo build -p miru-agent --features test`, `cargo clippy -p miru-agent --features test --all-targets -- -D warnings`, `cargo fmt -- --check`, `cargo machete`, and `cargo test -p miru-agent --features test` (1354 integration + 144 unit tests, incl. the 8 new uploads tests and T6) ALL PASS. `Preflight clean` NOT yet confirmed (later step).
- **Commits** (branch `feat/uploads-file-discovery`): `feat(workers): add upload file discovery + readiness worker (M2)` → `refactor(workers): make uploads decide_ready a public testable seam` → `test(workers): cover uploads discovery, readiness, cadence, dedupe (M2)`.
- **Scope held**: no digest/sha256, no `POST /uploads`/presigned PUT/confirm, no ledger persistence, no `delete_policy` enforcement, no outbound HTTP. Finalization-marker is a comment-only HOOK in `decide_ready`.
