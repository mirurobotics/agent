...
- [x] M3: `rust-lld` as the MSVC linker — kept (see Decision Log).
- [x] M4: cache workspace crates (`backend-api`, `device-api`) with a source-keyed cache — rejected on measured ceiling (see Decision Log).
- [x] Final: measurement table complete, losers reverted, preflight `CLEAN` (PR #241 stays draft until the caller promotes it).
...
- 2026-09-16 (M0, run 35133302125): with no lever applied, `mod.rs` took 73.67s versus 108.5s at baseline and 108.70s one run earlier; run-to-run noise on `windows-latest` is on the order of 30s for the test phase and ~10s for the compile phase. Judge levers on repeated runs or per-unit timings, not single wall-clock deltas.
- 2026-09-16 (M1, run 35133866291): `RUST_TEST_THREADS=16` failed `workers::poller::run::ignored_syncer_events` (`agent/tests/workers/poller.rs:374`), a wall-clock assertion with a 1s drift tolerance; oversubscribing 4 cores 4x stretched the gap between loop iterations past that. `mod.rs` was not faster either (79.62s vs 73.67s). 8 threads (run 35134395736) failed the same test and took 246.66s.
- 2026-09-16 (M2, run 35135500002): the workflow was refused at startup (`startup_failure`, no jobs). The repo's Actions policy is `allowed_actions: selected` with an allowlist (`taiki-e/install-action`, `Swatinem/rust-cache`, `cargo-bins/cargo-binstall`, `dtolnay/rust-toolchain`, ...) that does not include `samypr100/setup-dev-drive`, and `samypr100` is not a verified creator. Changing the allowlist is an infra/org setting outside this plan, so M2 creates the Dev Drive inline with the same cmdlets the action runs (`New-VHD | Mount-VHD -NoDriveLetter | Initialize-Disk | New-Partition`, `Format-Volume -DevDrive`, `Add-PartitionAccessPath`) and exports `TEMP`/`TMP` via `GITHUB_ENV`. No dismount post-step is needed on a discarded runner.
- 2026-09-16 (M3 timings): every warm `cargo test` invocation spends ~20s between `Updating crates.io index` and the first `Compiling` line (run 35136981030: 18:52:31.8 -> 18:52:52.3), i.e. Cargo re-validates the sparse index over HTTP despite `--locked` and a warm registry cache (the pinned rust-cache keeps index entries for locked packages). That is more than M3 and M4 together; the Linux jobs very likely pay it too. Not one of this plan's levers; filed as a follow-up (`--offline` / `cargo fetch --locked` at step level so the cache key is unchanged).
- 2026-09-16 (M3 timings): the generated API crates are cheap to compile (`backend-api` 2.1s, `device-api` 0.8s); the compile phase is dominated by `miru-agent`'s own units (lib 14.7s, lib test 57.6s, `mod` 52.6s, bin 25.9s, overlapped on 4 cores). This is what sank M4.
...
- 2026-09-16 (M3): Kept `rust-lld`. Wall-clock is inside the noise band (build 1m27s vs 1m33s, job 3m30s vs 3m36s), so the decision rests on the `--timings` artifacts of the two warm runs, which are far less noisy: per-unit durations M2 (`link.exe`) -> M3 (`rust-lld`): lib test 57.6s -> 53.7s, `mod` 52.6s -> 51.1s, `logs_init_smoke` 3.5s -> 2.5s, `logs_init_locked` 3.4s -> 2.2s, bin 25.9s -> 26.1s, all units 93.8s -> 88.0s. Every linked unit but the bin got faster; about 6s per job, below the plan's 10-30s hope because link time is a small share of these units. Costs nothing, the first run after the key change was cold as expected, no `lld-link` errors.
- 2026-09-16 (M4): Rejected caching the generated API crates without a CI round. The `--timings` artifacts show `backend-api` compiling in 2.1s (1.9s in M3) and `device-api` in 0.8s, overlapped, with `miru-agent`'s own lib unit starting 2.1s after them; `libs/` is 78 files. The best case for M4 is therefore about 2s on the critical path, below run-to-run noise by construction, while it adds `hashFiles('libs/**')`, an mtime-stamping step, a `cargo clean --package miru-agent` step, a larger cache save, and one cold run per `libs/` regeneration. `miru-agent` itself always rebuilds regardless (see the `build.rs` surprise), so `cache-workspace-crates` has nothing else to offer. No round could show it winning, so it was not run.
...
Kept: M0 (single timed `cargo test --timings` step, runner probe, timings artifact), M2 (inline ReFS Dev Drive for `target` and `TEMP`/`TMP`), M3 (`rust-lld`). Rejected: M1 (`RUST_TEST_THREADS`, red twice and never faster), M4 (ceiling ~2s). Same test counts on every green run (448 / 0 / 1 / 1 / 1538 / 0 doctests), job id unchanged, Linux jobs untouched.

Final warm run (248a391, run 35136981030): job 3m30s, build 1m27s, run 45s, lib 9.80s, `mod` 33.67s, versus the 4m49s / 1m36s / ~3m50s / 12.1s / 108.5s baseline. The test-phase target (under ~1m30s) is met with room to spare; the whole-job target of ~2m30s is not: the remaining time is the ~1m27s compile of `miru-agent`'s own units (which always rebuild because of the `build.rs` rerun-if-changed bug), ~20s of crates.io index revalidation per cargo invocation, and ~1m of fixed setup (checkout, toolchain, 33s cache restore, 13s NASM, 9s dev drive). The two follow-ups filed from this plan (fix `build.rs` fingerprinting; run cargo offline in CI) attack the first two and are worth more than any remaining lever here.

| Round | Commit | Job total | Build | Run | lib | mod | Keep? |
| --- | --- | --- | --- | --- | --- | --- | --- |
| Baseline | 4b3ac66 | 4m49s | 1m36s | ~3m50s step | 12.1s | 108.5s | n/a |
| M0 | 246838d | 4m20s | 1m38s | 86s | 10.81s | 73.67s | yes |
| M1 (16 thr) | 0a27e19 | 4m04s red | 1m20s | 91s | 10.85s | 79.62s (1 fail) | no |
| M1 (8 thr) | 0bde612 | 9m28s red | 1m35s | 263s | 16.32s | 246.66s (1 fail) | no |
| M2 | ae50979 | 3m36s | 1m33s | 41s | 7.82s | 31.63s | yes |
| M3 (warm) | 248a391 | 3m30s | 1m27s | 45s | 9.80s | 33.67s | yes |
| M4 | not run | - | - | - | - | - | no |

Retrospective: the plan's ranking held for M2 (the dominant win) but its expectations for M1 and M3 were off, and the split-step scaffolding in M0 exposed a pre-existing build-script bug that the plan's "no production code" boundary then had to work around. The Actions allowlist should have been checked at authoring time before proposing a third-party action; the inline cmdlets were a straightforward substitute.
...
| M3 (cold) | c40970c | 6m58s | 3m57s | 35s | 8.11s | 25.58s | (No cache found) | n/a |
| M3 | 248a391 | 3m30s | 1m27s | 45s | 9.80s | 33.67s | ~457 MB | yes |
| M4 | not run | - | - | - | - | - | - | no (ceiling ~2s, see Decision Log) |
