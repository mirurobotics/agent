...
- Observation (main merge, 2026-07-17): `origin/main` advanced with the upload subsystem (uploader, scan→upload bridge, persisted queue, `scan_bridge` → `sync_scan_bridge` rename) and PR #168 added a required `delete_policy: DeletePolicy` field to `scan::state::StableFile`, breaking the CI merge ref at our new prune test fixture. Merged main in (clean auto-merge, no conflicts) and set `delete_policy: DeletePolicy::Never` on the fixture in `scanner.rs`. Verified main's new upload code has no consumer of the removed prune APIs. Post-merge covgates: `scan` 99.09 ≥ 98.83, `workers` 85.56 ≥ 84.67, `upload` 97.1 ≥ 97.00 — no regeneration needed.
