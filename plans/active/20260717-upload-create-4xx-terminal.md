...
- Decision: apply the classified wrapper to both backend calls the executor makes — `create_upload` (required by the task) and `confirm_upload` (natural extension).
  Rationale: both wrap `http::with_retry` and map through the same `executor_err`; excluding confirm would need an artificial second error path. A terminal 4xx on confirm is equally unrecoverable by retrying. Note there is no separate vend-credentials call: credentials arrive embedded in the create response (`UploadWithCredentials`), and `agent/src/http/uploads.rs` defines only `create` and `confirm`. The `token()` step keeps the unclassified `executor_err` so authentication errors can never be classified terminal (preserves the 401/token-refresh carve-out end to end). Date/Author: 2026-07-17 / ben@miruml.com.
