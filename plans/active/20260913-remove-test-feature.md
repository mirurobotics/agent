...
- [x] Record the user's authorization for three additional CI rounds and prepare the minimal token-call enum repair.
- [ ] Complete refinement and preflight on the pushed final commit (additional round 1 of 3, overall round 4, pending for the `Copy` repair).
...
- 2026-09-13: After preflight reached `CAPPED` at the original three-round limit, the user authorized three additional CI rounds. Count the next published repair as additional round 1 of 3 (overall round 4), retain draft status, and keep this plan active until final-head acceptance succeeds.
...
The test implementation strengthens the six existing retry cases and two existing S3 replay cases and adds the inline `http::retry::tests::retry_delay_from_nanos_respects_jitter_boundaries` case. Equivalent-Linux inventory is 1,989 library cases (`354 + 1,640 - 6 + 1`), six `http_retry` cases, and one case in each logging target: 1,997 total, verified in round 1 below. File-scoped `rustfmt --check --edition 2021 --config skip_children=true` passed for the four edited Rust files, and `git diff --check` passed. Static inspection confirmed all six retry names remain and the `.covgate` diff is empty. Final-head refinement and full CI validation remain pending. No local compilation/full tests, threshold changes, generated-library edits, commits, or pushes were performed during the test implementation step. This plan remains active until the final acceptance conditions are met.

Preflight round 1, [CI run 34776775613](https://github.com/mirurobotics/agent/actions/runs/34776775613) at `db5e7e1b1b1527555c9ced1f796e05aaf08d0037`, passed the Linux production check, ordinary Cargo and wrapper tests, inventory recording, coverage wrappers, Windows production check, and tools checks. Only lint failed, stopping at the 65 split-import diagnostics. The correction batch groups those imports in all 30 reported files. File-scoped rustfmt checks passed; a static comparison confirmed all 90 imported paths and aliases and every non-import byte are unchanged.

A fresh target-aware sorted-multiset comparison of complete baseline run `34647851771`, test job `103422810288`, and round 1 run `34776775613`, test job `103776213356`, verified all 1,996 original cases are preserved with no missing, duplicate, or ignored cases. Final library names beginning with `tests::` were normalized once to the old `mod` target, external retry names were mapped back to the old `http::retry::` names, and source-inline and logging cases retained their targets and names. The sole addition is `http::retry::tests::retry_delay_from_nanos_respects_jitter_boundaries`, giving 1,997 cases. All five round 1 execution passes match that inventory, the HTML coverage index exists, all 37 agent coverage gates and eight tool gates passed unchanged, and the tool suite passed all 123 cases.

Preflight round 2, [CI run 34777572977](https://github.com/mirurobotics/agent/actions/runs/34777572977) at `65d99c01d935640f7739a1e15bf6b54a908ae24c`, validated the import-only correction batch. Its test/coverage job `103778385655`, Windows production check, and tools passed; lint alone failed with eight Clippy dead-code diagnostics. The complete failed-job output was read before the local correction. The correction removes unused mock conveniences and obsolete state, preserves supported error injection and recording, asserts cancellation of the scanner's first sleep, and checks refresh-before-read token call ordering in existing synchronized tests. File-scoped rustfmt checks passed for all eight edited Rust files, and `git diff --check` passed. No cases or existing assertions were removed or skipped, so the expected inventory remains 1,997.

Preflight round 3, [CI run 34778146199](https://github.com/mirurobotics/agent/actions/runs/34778146199) at `3c35a949d2d44f4acb56d3235bbae03876276b34`, passed both production checks and the tools job. Test job `103779967216` and lint job `103779967343` both failed with the same E0277 at `agent/tests/workers/token_refresh.rs:87`; compilation stopped before the runtime inventory and coverage steps. The complete output of both failing jobs was read before the repair. The sole code change adds `Copy` to the unit-only `TokenManagerCall` enum's derives; call histories, assertions, test cases, and all 37 agent coverage gates remain unchanged. The original preflight budget ended `CAPPED`; the user then authorized three additional rounds. Additional round 1 of 3 (overall round 4) and final-head CI acceptance remain pending, and this plan remains active.

The additional-round repair passed file-scoped `rustfmt --check --edition 2021 --config skip_children=true agent/tests/mocks/token_manager.rs` and `git diff --check`. Static inspection found no individual `TokenManagerCall` clones to adjust, and the `.covgate` diff against `main` is empty. No local compilation, tests, Clippy, aggregate lint, commits, or pushes were performed during this repair step.
