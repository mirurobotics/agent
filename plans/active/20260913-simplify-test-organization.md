...
- [x] 2026-09-13: Diagnose CI round 1 at `675882dd2d9bcf1568a2087e8224c5fb060c3d88`, run `34782193470`, from the complete failed-job logs. Linux/Windows production checks and tools passed; library-test compilation failed to resolve filesystem fixture children, and integration import lint reported 17 grouped multi-anchor imports. Move the unchanged filesystem module to `agent/tests/test_utils/filesys/mod.rs`, update the unit adapter path, and split those imports by top-level anchor. One of three preflight CI rounds is used; two remain. Runtime identities, coverage gates, and HTML remain unvalidated.
- [x] 2026-09-13: Round 1 fix-stage static checks passed: format all 19 touched Rust files individually with child traversal disabled, compare the moved module byte-for-byte with its prior contents, inspect all 17 import edits against the linter's normalization rules, and run `git diff --check`. The diff preserves test bodies and imported symbols. No local build, test, or custom-lint run was performed; the linter binary is absent.
- [ ] Milestone 3: Complete fresh reviews and exact-head CI/delivery validation after the round 1 fixes.
...
CI round 1 failed before agent test execution and reports. Both diagnosed failure classes have scoped fixes prepared; final runtime identity comparison, coverage-selection evidence, and exact-head delivery validation remain pending. Preflight budget: one round used, two remaining.
