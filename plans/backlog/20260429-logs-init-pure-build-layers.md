...
- Decision: Keep two top-level binaries (was five): `logs_init_smoke.rs` (combines successful global install + `LogsErr::SetGlobalDefault` on second install in one `#[serial]` test) and `logs_init_locked.rs` (RUST_LOG-locked branch through real `init`). Drop `logs_init_reload.rs`, `logs_init_stdout.rs`, `logs_init_file_only.rs`, `logs_init_double.rs`.
  Rationale: Smoke + double-install fit in one process — install succeeds, second install legitimately observes the already-installed subscriber. The locked binary must stay separate because the shared integration binary inherits `RUST_LOG=off` from `scripts/test.sh`, blocking the not-locked branch there. The three dropped binaries cover layer-construction and reload-handle mechanics, which `build_layers` makes reachable from `tests/logs/mod.rs` without spawning new processes.
  Date/Author: 2026-04-29, ben@miruml.com.
...
2. Add a public function `build_layers` that contains all of `init`'s logic except `set_global_default`:
   ...
   Fallback if `Layer::and_then` is unavailable in the locked `tracing-subscriber` version: return `(reload_layer, BoxedLogLayer /* fmt */, WorkerGuard, ReloadHandle, bool)` and let `init` compose with `Registry::default().with(reload_layer).with(fmt_layer)`. The fmt layer must still be boxed because the stdout/file branches produce different concrete types. Pick whichever compiles cleanly; the signature is internal — `tests/logs/mod.rs` adapts.
...
2. The matrix should cover:
   ...
   - `test_build_layers_stdout_debug` — `Options { stdout: true, log_level: LogLevel::Debug, log_dir: <tempdir> }`. Assert `handle.reload(EnvFilter::new("warn")).is_ok()` and that attaching the composite via `tracing::subscriber::with_default(Registry::default().with(layer), || tracing::warn!("hello"))` does not panic. Proves the returned tuple is well-formed.
   - `test_build_layers_file_only_warn` — same shape with `stdout: false, log_level: LogLevel::Warn`.
   - `test_build_layers_respects_rust_log_when_set` — set `RUST_LOG=info`, call `build_layers`, assert trailing bool is `true`. Mark `#[serial_test::serial(rust_log)]` because env vars are process-wide. Use `unsafe { std::env::set_var(...) }` with a Drop-based RAII guard that restores the prior value (`RUST_LOG=off` from `scripts/test.sh`) on test exit.
   - `test_build_layers_uses_options_when_rust_log_unset` — same RAII pattern; clear `RUST_LOG`, call `build_layers`, assert trailing bool is `false`. Also `#[serial(rust_log)]`.
   - `test_build_layers_reload_handle_changes_filter` — call `build_layers` with `stdout: true`, attach via `with_default`, use the reload handle to flip the filter, and assert via a `CapturingWriter` (pattern already in this file) that pre-reload `debug!` is filtered and post-reload is emitted. If wiring a capturing writer through the boxed composite proves awkward, narrow the assertion to `handle.reload(...).is_ok()` before and after — emission semantics are still covered by the existing `test_reload_level_changes_filter`.
...
- Milestone 1's source edit is idempotent. If `cargo build` fails on the type signature, use the fallback shape in Plan of Work § Milestone 1 step 2.
