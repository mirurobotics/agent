...
echo ""
echo "Checking per-module coverage (modules discovered by .covgate under src/)..."
echo ""
...
while read -r covgate_file; do
    ...
    # If .covgate is directly in SRC_DIR, display as "src"
    if [ "$module_display" = "$module_path" ]; then
        module_display="src"
    fi
    module_dir_abs="$module_path/"
